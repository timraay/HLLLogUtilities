...
from lib.autosession import (
    MAX_DURATION_MINUTES,
    MIN_PLAYERS_TO_START,
    MIN_PLAYERS_UNTIL_STOP,
...
class RCONCredentialsModal(Modal):
    name = ui.TextInput(
        label="Server Name - How I should call this server",
        placeholder="My HLL Server #1",
        required=True,
        max_length=60,
    )
    address = ui.TextInput(
        label="RCON Address - To connect to RCON",
    ...
    password = ui.TextInput(
        label="RCON Password - For authentication", required=True, max_length=40
    )
    ...
    async def on_submit(self, interaction: Interaction):
        ...
        if not MIN_ALLOWED_PORT <= port <= MAX_ALLOWED_PORT:
            raise CustomException(
                "Invalid port!", f"Port {port} is out of range 1025-65536."
            )

        password = self.password.value
        if " " in password:
            raise CustomException(
                "Invalid password!", "Password must not contain spaces."
            )
        ...
        logging.info(
            "Attempting connection to %s:%s (GID: %s)",
            address,
            port,
        ...
        try:
            ...
            logging.info(
                "Successfully opened connection with %s:%s (GID: %s)",
                address,
                port,
                interaction.guild_id,
            )
...
class AutoSessionView(View):
    def __init__(self, credentials: Credentials, guild: discord.Guild):
        ...
        if self.enabled:
            self.button = CallableButton(
                self.disable_button_cb, label="Disable", style=discord.ButtonStyle.red
            )
        else:
            self.button = CallableButton(
                self.enable_button_cb, label="Enable", style=discord.ButtonStyle.green
            )

        self.add_item(self.button)
        self.add_item(
            CallableButton(
                self.select_modifiers_cb,
                label="Modifiers...",
        ...
        self.add_item(
            ui.Button(
                label="Docs",
                style=discord.ButtonStyle.blurple,
                url="https://github.com/timraay/HLLLogUtilities#automatic-session-scheduling",
            )
        )
        self.add_item(
            CallableButton(self.update, emoji="🔄", style=discord.ButtonStyle.gray)
        )
    ...
    def get_embed(self):
        ...
        if self.credentials.autosession_enabled:
            ...
            if session:
                ...
                embed.add_field(name="Status", value=r"\🟢Enabled", inline=True)
                embed.add_field(
                ...
                embed.add_field(name="Status", value=r"\🟠Problems", inline=True)
                embed.add_field(
                ...
                embed.add_field(
                    ...
                    value=self.autosession.last_error,
                    inline=False,
                )
                ...
                embed.add_field(name="Status", value=r"\🟡Cooldown", inline=True)
                embed.add_field(
                    name="Waiting for players...",
                ...
                ts = int(
                    (
                        ...
                        + self.autosession._cooldown
                        * timedelta(seconds=SECONDS_BETWEEN_ITERATIONS)
                    ).timestamp()
                )
                embed.title = "AutoSession is currently enabled."
                embed.color = discord.Colour.from_rgb(255, 243, 33)
            ...
            embed.add_field(name="Status", value=r"\🔴Disabled", inline=True)
            embed.add_field(
                name="Documentation",
        ...
        if modifiers:
            embed.add_field(
                name=f"Enabled modifiers ({len(modifiers)})",
                value="\n".join(
                    [
    ...
    async def select_modifiers_cb(self, interaction: Interaction):
        view = SessionModifierView(
            self.message,
            self.updated_modifiers_cb,
            flags=self.credentials.default_modifiers,
        )
        await interaction.response.edit_message(
            content="Select all of the modifiers you want to enable by clicking on the buttons below",
            view=view,
            embed=None,
        )

    @__credentials_still_exist
    async def updated_modifiers_cb(
        self, interaction: discord.Interaction, modifiers: ModifierFlags
    ):
        ...
        if changed and session and (modifiers != session.modifier_flags):
            ...
            async def do_edit_ongoing_session(interaction: Interaction):
                ...
                await interaction.response.edit_message(
                    content=None, view=self, embed=self.get_embed()
                )
            ...
            view.add_item(
                CallableButton(
                    skip_edit_ongoing_session, style=ButtonStyle.gray, label="Skip"
                )
            )
...
class SessionModifierView(View):
    ...
    async def toggle_value(
        self, interaction: Interaction, flags: ModifierFlags, enable: bool
    ):
        if enable:
            self.flags |= flags
        else:
    ...
    def update_self(self):
        ...
        self.add_item(
            CallableButton(self.callback, label="Back...", style=ButtonStyle.gray)
        )
...
async def autocomplete_credentials(interaction: Interaction, current: str):
    choices = [
        app_commands.Choice(name=str(credentials), value=credentials.id)
        for credentials in await credentials_in_guild_tll(interaction.guild_id)
        if current.lower() in str(credentials).lower() and credentials.id
    ]
...
class credentials(commands.Cog):
    ...
    Group = app_commands.Group(
        name="credentials", description="Manage your credentials"
    )
    ...
    async def list_credentials(self, interaction: Interaction):
        ...
        embed = discord.Embed(
            ...
            if all_credentials
            else "No credentials are known yet.",
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)

    @Group.command(name="remove", description="Remove credentials")
    @app_commands.describe(credentials="Credentials for RCON access")
    @app_commands.autocomplete(credentials=autocomplete_credentials_no_custom)
    async def remove_credentials(self, interaction: Interaction, credentials: int):
        ...
        async def _delete_credentials(_interaction: Interaction, do_edit=True):
            ...
            for session in scheduled:
                session.logger.info(
                    "Deleting scheduled session since its credentials are also being deleted"
                )
                session.delete()
    ...
    async def add_credentials(self, interaction: Interaction):
        ...
        async def on_form_submit(
            _interaction: Interaction, name: str, address: str, port: int, password: str
        ):
            credentials = Credentials.create_in_db(
                _interaction.guild_id,
                name=name,
                address=address,
                port=port,
                password=password,
    ...
    async def edit_credentials(self, interaction: Interaction, credentials: int):
        ...
        modal = RCONCredentialsModal(
            on_form_submit, title="RCON Credentials Form", defaults=credentials
        )
        await interaction.response.send_modal(modal)

    @app_commands.command(
        name="autosession", description="Manage AutoSession for a server"
    )
    @app_commands.describe(credentials="A server's credentials")
    @app_commands.autocomplete(credentials=autocomplete_credentials_no_custom)
