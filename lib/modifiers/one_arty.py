...
from hllrcon import HLLWeapon, WeaponType
...
def is_arty(weapon_name: str, yes_no: bool = True):
    try:
        weapon = HLLWeapon.by_id(weapon_name)
    except ValueError:
        return not yes_no
...
def is_arty_condition(yes_no: bool = True):
    def decorator(func):
        return add_condition(lambda _, event: is_arty(event.weapon, yes_no=yes_no))(
            func
        )
...
class OneArtyModifier(Modifier):
    ...
    async def initialize(self, event: ActivationEvent):
        self.dap: dict[int, str | None] = {1: None, 2: None}
        self.expire_tasks: dict[int, asyncio.Task | None] = {1: None, 2: None}
    ...
    async def punish_ten_people(self, player: Player, reason: str):
        ...
        self.logger.info(
            "Punished %s/%s players: %s",
            len(punished),
            len(players),
            ", ".join([f"{player.name} ({player.id})" for player in punished]),
        )
    ...
    async def assign_players_to_arty(self, event: PlayerKillEvent):
        ...
        await self.get_rcon().client.message_player(
            player_id=player.id, message=message
        )

        commander = event.snapshot.teams[team.id - 1].get_commander()
        if commander:
            message = (
                f"{player.name} has become your team's designated artillery player!"
            )
            await self.get_rcon().client.message_player(
                player_id=commander.id, message=message
            )
    ...
    @add_condition(
        lambda mod, event: (
            mod.dap[event.get_player().team_id] and not mod.is_dap(event.get_player())
        )
    )
    ...
    async def expire_dap_after_cooldown(self, player: Player, team_id: int):
        try:
            ...
            self.logger.exception(
                "Failed to properly unassign arty player after cooldown"
            )
    ...
    async def punish_killing_a_dap(self, event: PlayerKillEvent):
        ...
        if self.is_dap(victim):
            reason = (
                "You are not allowed to kill the enemy's designated artillery player!"
            )
            await self.punish_ten_people(player, reason=reason)
