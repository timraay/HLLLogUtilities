...
from utils import side_by_side, toTable
...
class PlayerScore(BaseModel):
    ...
    def __sub__(self, other: "PlayerScore"):
        if not isinstance(other, PlayerScore):
            return NotImplemented
        return PlayerScore(
            **combine_dicts(self.model_dump(), other.model_dump(), op=operator.sub)
        )


class MatchGroup:
    def __init__(self, matches: list["MatchData"] | None = None):
        self.matches: list[MatchData] = list(matches) if matches else []
    ...
    def from_logs(cls, logs: list["LogLine"]):
        ...
        while logs:
            ...
            if log_type == EventTypes.server_match_start and match_logs:
    ...
    def get_matches_for_player(self, player: Union["PlayerData", str]):
        ...
        if not player_data:
            raise ValueError(f"Player with Steam ID {player} could not be found")
    ...
    def avg_match_length(self):
        return (
            (self.total_match_length / self.num_matches_played)
            if self.num_matches_played
            else timedelta()
        )

    @property
    def shortest_match(self):
        return min(self.matches, key=lambda m: m.duration)

    @property
    def winner_kills(self):
        return sum(
            (
                match.total_allied_kills
                if match.winner == EFaction.Allies
                else match.total_axis_kills
            )
            for match in self.matches
        )
    ...
    def winner_deaths(self):
        return sum(
            (
                match.total_allied_deaths
                if match.winner == EFaction.Allies
                else match.total_axis_deaths
            )
            for match in self.matches
        )
...
class DataStore:
    ...
    @staticmethod
    def map_weapons(
        weapons: dict[AnyWeapon, int],
        *mappings: dict[AnyWeapon, str],
        skip_unmapped=False,
    ) -> dict[str, int]:
        if not mappings:
            return {weapon.name: value for weapon, value in weapons.items()}
    ...
    def weapons_killed_with(
        self, *mappings: dict[AnyWeapon, str], skip_unmapped=False
    ) -> dict[str, int]:
        all_weapons: dict[AnyWeapon, int] = {}
    ...
    def weapons_teamkilled_with(self, *mappings, skip_unmapped=False) -> dict[str, int]:
        weapons_killed_with: dict[AnyWeapon, int] = {}
        for player in self.players:
            weapons_killed_with = combine_dicts(weapons_killed_with, player.weapons)

        weapons_died_to: dict[AnyWeapon, int] = {}
        for player in self.players:
            weapons_died_to = combine_dicts(weapons_died_to, player.weapons)

        all_weapons: dict[AnyWeapon, int] = {
            name: amount
            for name, amount in combine_dicts(
                weapons_died_to, weapons_killed_with, operator.sub
            ).items()
            if amount > 0
        }
        return self.map_weapons(all_weapons, *mappings, skip_unmapped=skip_unmapped)
    ...
    def __add__(self, other: "DataStore") -> "DataStore":
        ...
        players = combine_dicts(
            {p.player_id: p for p in self.players},
            {p.player_id: p for p in other.players},
        )
    ...
    def to_text(self, single_match: bool = False):
        data = sorted(
            filter(lambda player: player.player_id, self.players),
        ...
        if single_match:
            output = "{: <5} {: <25} {: <6} {: <6} {: <5} {: <5} {: <5} {: <6} {: <27} {: <25} {: <25} {: <4} {: <4} {: <4} {: <4} {}".format(
                *headers
            )
        else:
            output = "{: <6} {: <28} {: <6}  {: <25} {: <6} {: <6} {: <5} {: <5} {: <5} {: <6} {: <27} {: <25} {: <25} {: <4} {: <4} {: <4} {: <4} {: <9} {}".format(
                *headers
            )
...
class MatchData(DataStore):
    ...
    def from_logs(cls, logs: list["LogLine"]):
        if not logs:
            return cls(players=[], duration=timedelta(), game=Game.HLL)

        # dirtyyyy
        if any(log.player_id and not log.player_id.startswith("0002") for log in logs):
            game = Game.HLL
        else:
            game = Game.HLLV
        ...
        for log in logs:
            ...
            if log_type == EventTypes.server_map_change:
                if match_ended:
                    ...
                    map_name = mappings.parse_layer(log.old, game).pretty_name
                else:
                    assert log.new is not None
                    map_name = mappings.parse_layer(log.new, game).pretty_name
                continue
            ...
            if log.player_id in data:
                ...
                killer_data = PlayerData(
                    log.player_id, log.player_name, logs_start, logs_end
                )
                data[log.player_id] = killer_data

            if log.player2_id in data:
                ...
                victim_data = PlayerData(
                    log.player2_id, log.player2_name, logs_start, logs_end
                )
            ...
            killer_faction = (
                EFaction(log.player_team) if log.player_team else EFaction.Any
            )
            victim_faction = (
                EFaction(log.player2_team) if log.player2_team else EFaction.Any
            )

            if log.weapon:
                weapon_cls = game_switch(game, HLLWeapon, HLLVWeapon)
                try:
                    weapon = weapon_cls.by_id(log.weapon)
                except ValueError:
                    faction_cls = game_switch(game, HLLFaction, HLLVFaction)
                    weapon = weapon_cls(
                        id=log.weapon,
                        name=log.weapon,
                        type=WeaponType.UNKNOWN,
                        factions=set(faction_cls.all()),
                    )
            else:
                weapon = None
        ...
        if include_unknown:
            return DataStore(
                ...
                [
                    ...
                    or player.faction == EFaction.Any
                    or player.faction is None
                ],
            )
        else:
            return DataStore(
                self.duration,
                [player for player in self.players if player.faction == faction],
            )
    ...
    def to_dict(self) -> dict[str, Any]:
        out = {
            "map": self.map,
            "team1": {"faction": "Allies", "score": self.team1_score},
            "team2": {"faction": "Axis", "score": self.team2_score},
        }
...
class PlayerData:
    def __init__(
        ...
        self.weapons: dict[AnyWeapon, int] = {}
        self.causes: dict[AnyWeapon, int] = {}
    ...
    def __eq__(self, other):
        return (
            ...
            if isinstance(other, PlayerData)
            else NotImplemented
        )
    ...
    def update_faction(self, faction: EFaction):
        ...
        if faction != EFaction.Any:
            if self._faction == EFaction.Allies:
                self.allied_seconds_played += (
                    self.seconds_played - self._last_seen_playtime
                )
                self._last_seen_playtime = self.seconds_played

            elif self._faction == EFaction.Axis:
                self.axis_seconds_played += (
                    self.seconds_played - self._last_seen_playtime
                )
                self._last_seen_playtime = self.seconds_played
    ...
    def kill(self, victim: "PlayerData", weapon: AnyWeapon, faction: EFaction):
        self.kills += 1
        if faction == EFaction.Allies:
            self.allied_kills += 1
        elif faction == EFaction.Axis:
        ...
        # Killstreak
        self._curr_streak += 1
        self.killstreak = max(self.killstreak, self._curr_streak)
        # Deathstreak
    ...
    def teamkill(self, victim: "PlayerData", weapon: AnyWeapon, faction: EFaction):
        self.teamkills += 1
    ...
    def death(
        self, nemesis: "PlayerData", weapon: AnyWeapon, faction: EFaction | None = None
    ):
        self.deaths += 1
        if faction == EFaction.Allies:
        ...
        self.deathstreak = max(self.deathstreak, self._curr_deathstreak)
    ...
    def kills_per_minute(self) -> float:
        return (
            round(self.kills / (self.seconds_played / 60), 2)
            if self.seconds_played
            else self.kills
        )

    @property
    def weapon(self) -> AnyWeapon | None:
        return (
            max(self.weapons, key=lambda x: self.weapons[x]) if self.weapons else None
        )

    @property
    def cause(self) -> AnyWeapon | None:
        return max(self.causes, key=lambda x: self.causes[x]) if self.causes else None

    @property
    def victim(self) -> str | None:
        return (
            max(self.victims, key=lambda x: self.victims[x]) if self.victims else None
        )

    @property
    def nemesis(self) -> str | None:
        return (
            max(self.nemeses, key=lambda x: self.nemeses[x]) if self.nemeses else None
        )
    ...
    def seconds_alive(self) -> int:
        return min(
            self.seconds_played,
            (1 + (self.score.offense + self.score.defense) // 20) * 60,
        )
    ...
    def to_string(self, rank: int, single_match=True) -> str:
        weapon = (
            mappings.FACTIONLESS.get(self.weapon, self.weapon.name)
            if self.weapon
            else "None"
        )
        weapons = DataStore.map_weapons(
            self.weapons, mappings.VEHICLE_WEAPONS_FACTIONLESS, mappings.FACTIONLESS
        )
        victim = self.victim
        ...
        if single_match:
            ...
            return "#{: <5} {: <32} {: >2}  {: <25} {: <6} {: <6} {: <5} {: <5} {: <5} {: <6} {: <28}{: <25} {: <25} {: <4} {: <4} {: <4} {: <4} {: <9} {}".format(
                rank,
                self.player_id,
    ...
    def to_dict(self):
        return {
            ...
            "weapons_used": {
                k.id: v
                for k, v in sorted(
                    self.weapons.items(), key=lambda x: x[1], reverse=True
                )
            },
            "weapons_died_to": {
                k.id: v
                for k, v in sorted(
                    self.causes.items(), key=lambda x: x[1], reverse=True
                )
            },
            "victims": {
                k: v
                for k, v in sorted(
                    self.victims.items(), key=lambda x: x[1], reverse=True
                )
                if k != "None"
            },
            "nemeses": {
                k: v
                for k, v in sorted(
                    self.nemeses.items(), key=lambda x: x[1], reverse=True
                )
                if k != "None"
            },
...
def _get_weapon_stats(stats: DataStore):
    def get_weapons_table(*mappings, skip_unmapped=False, title=None, show_tks=False):
        ...
        for weapon, kills in sorted(weapons.items(), reverse=True, key=lambda i: i[1]):
            table.append(
                [weapon, kills, str(round(kills * 100 / (total_kills or 1), 2)) + "%"]
            )
    ...
    weaponsTable = get_weapons_table(
        mappings.VEHICLE_WEAPONS_FACTIONLESS, mappings.FACTIONLESS, show_tks=True
    )

    weaponsBasicAlliesTable = get_weapons_table(
        mappings.BASIC_CATEGORIES_ALLIES,
        skip_unmapped=True,
        title="WEAPONS USED BY ALLIES",
    )
    weaponsBasicAxisTable = get_weapons_table(
        mappings.BASIC_CATEGORIES_AXIS, skip_unmapped=True, title="WEAPONS USED BY AXIS"
    )

    table: list[list[str | int]] = [["WEAPON", "ALLIES", "AXIS", "TOTAL", "RATE"]]
    weapons_allies = stats.weapons_killed_with(
        mappings.BASIC_CATEGORIES_ALLIES, skip_unmapped=True
    )
    weapons_axis = stats.weapons_killed_with(
        mappings.BASIC_CATEGORIES_AXIS, skip_unmapped=True
    )
    ...
    for weapon, kills in sorted(weapons.items(), reverse=True, key=lambda i: i[1]):
        ...
        table.append(
            [
                ...
                axis_kills,
                kills,
                str(round(kills * 100 / (total_kills or 1), 2)) + "%",
            ]
        )
    weaponsBasicTable = toTable(table, title="FACTIONS SIDE TO SIDE")

    vehiclesTable = get_weapons_table(
        mappings.VEHICLE_CLASSES, skip_unmapped=True, title="VEHICLES USED"
    )
    vehiclesAlliesTable = get_weapons_table(
        mappings.VEHICLES_ALLIES, skip_unmapped=True, title="ALLIED VEHICLES"
    )
    vehiclesAxisTable = get_weapons_table(
        mappings.VEHICLES_AXIS, skip_unmapped=True, title="AXIS VEHICLES"
    )

    return side_by_side(
        weaponsTable,
        "\n\n\n".join(
            [weaponsBasicAlliesTable, weaponsBasicAxisTable, weaponsBasicTable]
        ),
...
def create_scoreboard(data: Union["MatchData", "MatchGroup"]):
    ...
    table = [
        ...
        [
            "Allies",
            stats.total_allied_kills,
            stats.total_allied_deaths,
            round(stats.total_allied_kills / (stats.total_allied_deaths or 1), 2),
            total_allied_score.combat,
            total_allied_score.offense,
            total_allied_score.defense,
            total_allied_score.support,
            f"{round(100 * stats.total_allied_time_alive / (stats.total_allied_time_played or stats.total_allied_time_alive), 1)}%",
        ],
        [
            "Axis",
            stats.total_axis_kills,
            stats.total_axis_deaths,
            round(stats.total_axis_kills / (stats.total_axis_deaths or 1), 2),
            total_axis_score.combat,
            total_axis_score.offense,
            total_axis_score.defense,
            total_axis_score.support,
